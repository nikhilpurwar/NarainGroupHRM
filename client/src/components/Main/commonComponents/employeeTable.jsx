...
const EmployeeTable = ({
    ...
    // Socket.IO real-time updates for monthly summaries
    useEffect(() => {
        if (!renderActions) return

        const apiUrl = import.meta.env.VITE_API_URL ?? 'http://localhost:5100'
        const socket = clientIO(apiUrl, {
        ...
        socket.on("connect", () => {
            console.log("✓ EmployeeTable Socket.IO connected");
        });

        socket.on("connect_error", (err) => {
            console.error("EmployeeTable Socket.IO connection error:", err);
        });

        socket.on("attendance:updated", async (payload) => {
            console.debug('EmployeeTable socket attendance:updated', payload);
            if (!payload || !payload.employee) return;

            // If summary is provided in payload, use it directly
            if (payload.summary) {
                setSummaryMap(prev => ({
                    ...prev,
                    [payload.employee]: {
                        present: payload.summary.totalPresent || 0,
                        absent: payload.summary.totalAbsent || 0
                    }
                }))
                return;
            }

            // Fallback: Refresh the summary for this employee from API
            try {
                const token = typeof window !== 'undefined' ? (sessionStorage.getItem('token') || localStorage.getItem('token')) : null
                const headers = token ? { Authorization: `Bearer ${token}` } : {}
                ...
                const monthKey = `${currentYear}-${currentMonth}`
                const res = await axios.get(`${apiUrl}/api/monthly-summary`, {
                    params: { month: monthKey, employeeIds: payload.employee },
                    headers
                })

                if (res.data?.success) {
                    const bodyMap = res.data.data?.map || {}
                    const updatedSummary = bodyMap[payload.employee]

                    if (updatedSummary) {
                        setSummaryMap(prev => ({
                            ...prev,
                            [payload.employee]: {
                                present: updatedSummary.totalPresent || 0,
                                absent: updatedSummary.totalAbsent || 0
                            }
                        }))
                    }
                }
            } catch (err) {
                console.error('Error refreshing summary after attendance update:', err)
            }
        });

        socket.on("disconnect", () => {
            console.log("EmployeeTable Socket.IO disconnected");
        });
